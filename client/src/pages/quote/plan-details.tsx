...
export default function PublicPlanDetailsPage() {
  const [, navigate] = useLocation();
  const [, params] = useRoute("/q/:token/plan-details");
  const token = params?.token;
  const { data, isLoading, isError } = usePublicQuote(token);
